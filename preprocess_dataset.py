...
def parse_args():
    ...
    parser.add_argument('--origin-dir', default='/media/lyx/Ain/data/Shanghai/part_B_final',
                        help='original data directory')
    parser.add_argument('--data-dir', default='/media/lyx/Ain/data/Shanghai_BL/part_B_final',
...
if __name__ == '__main__':
    ...
    for phase in ['train_data', 'test_data']:
        ...
        if phase == 'train_data':
            sub_save_dir = os.path.join(save_dir, 'train_data', 'images')
            if not os.path.exists(sub_save_dir):
                os.makedirs(sub_save_dir)
            label_list = glob(os.path.join(sub_dir,'images', '*.jpg'))
            for i in label_list:
                im_path = os.path.join(sub_dir,'images', i.strip())
                # print(im_path)
                # im_path = im_path.replace('images','ground_truth')
            ...
            sub_save_dir = os.path.join(save_dir, 'test_data', 'images' )
            if not os.path.exists(sub_save_dir):
                os.makedirs(sub_save_dir)
